...
// 2020.04.03 dsites Added wait command
// 2024.05.29 dsites Added LLC miss tracking commands
//
...
void Usage() {
  ...
  fprintf(stderr, "  go, stop [<fname>], init, on, off, flush, reset, stat, dump, quit, wait <sec>\n");
  exit(0);
}
...
int main (int argc, const char** argv) {
  ...
  while (ReadLine(stdin, buffer, kMaxBufferSize)) {
    ...
    // NOTE: order for ipc/llc/wrap is alphabetical i-l-w
    else if (strcmp(buffer, "go") == 0) {
      control_flags = 0; 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (strcmp(buffer, "goipc") == 0) {
      control_flags |= DO_IPC; 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (strcmp(buffer, "gollc") == 0) {
      control_flags |= DO_LLC; 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (strcmp(buffer, "gowrap") == 0) {
      control_flags |= DO_WRAP; 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (strcmp(buffer, "goipcllc") == 0) {
      control_flags |= (DO_IPC | DO_LLC); 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (strcmp(buffer, "goipcwrap") == 0) {
      control_flags |= (DO_IPC | DO_WRAP); 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (strcmp(buffer, "gollcwrap") == 0) {
      control_flags |= (DO_LLC | DO_WRAP); 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (strcmp(buffer, "goipcllcwrap") == 0) {
      control_flags |= (DO_IPC | DO_LLC | DO_WRAP); 
      kutrace::DoReset(control_flags); kutrace::DoInit(argv[0]); kutrace::DoOn();
    } else if (memcmp(buffer, "stop", 4) == 0) {
      /* Pick off filename if any */
      if (strlen(buffer) > 5) {
        strcpy(fname, buffer + 5);
	// Append ".trace" if not already there
        if (strstr(fname, ".trace") == NULL) {
          strcpy(&fname[strlen(fname)], ".trace");
        }
      }
      /* After DoOff wait 20 msec for any pending tracing to finish */
      kutrace::DoOff(); msleep(20); 
      kutrace::DoFlush(); kutrace::DoDump(fname); control_flags = 0; kutrace::DoQuit();
    } else if (strcmp(buffer, "quit") == 0) {kutrace::DoQuit();}
    else if (strcmp(buffer, "exit") == 0) {kutrace::DoQuit();}
    else if (memcmp(buffer, "wait ", 5) == 0) {
      int n = atoi(buffer + 5);
      msleep(n * 1000);
    } else {
      fprintf(stdout, "Not recognized '%s'\n", buffer);
      fprintf(stdout, 
        "  go(ipc|llc|wrap)* stop init on off flush reset stat dump wait quit\n");
    }
