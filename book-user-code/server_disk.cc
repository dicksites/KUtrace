// server_disk.cc cloned from server4.cc 2018.07.08
...
int OpenSocket(int16 portnum) {
  ...
  //fprintf(stderr, "server_disk: Open server socket\n");
...
int ConnectToClient(int sockfd, uint32* client_ip, uint16* client_port) {
  ...
  //fprintf(stderr, "server_disk: listen server socket\n");
  ...
  *client_port = ntohs(sin->sin_port);
  ...
  fprintf(stderr, "server_disk: connection from %s\n", FormatIpPort(*client_ip, *client_port));
  return acceptsockfd;
}
...
//  if (fd < 0) {perror("server_disk write open"); return;}
...
bool DoRead(SharedData* shareddata, const RPC* request, RPC* response) {
  ...
  string value;
  {
    ...
    PutStringRPC(string((char*)ptr, n), response);
    free(rawptr);
...
bool DoChksum(SharedData* shareddata, const RPC* request, RPC* response) {
  const uint8* req_data = request->data; 
  string key = GetStringArg(&req_data);
  string value;
  {
...
void* SocketLoop(void* arg) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      kutrace::addevent(KUTRACE_RPCIDREQ, tempid);

      LogRPC(shareddata->logfile, &request);
      if (verbose) {
        fprintf(stdout, "server_disk: ReceiveRequest:   "); 
      ...
      if (strcmp(hdr->method, "ping") == 0) {
        ...
        ok &= DoRead(shareddata, &request, &response);
      } else if (strcmp(hdr->method, "chksum") == 0) {
        ok &= DoChksum(shareddata, &request, &response);
      ...
      if (verbose) {
        fprintf(stdout, "server_disk: SendResponse:     "); 
...
void Usage() {
  fprintf(stderr, "Usage: server_disk directory_name [portnumber] [num_ports] [-verbose] [-direct] [-data] [-wait msec]\n");
  exit(EXIT_FAILURE);
}
...
int main (int argc, const char** argv) {
  ...
  shareddata.directory = string(directory_name);
  ...
  for (int n = 0; n < num_ports; ++n) {
    ...
    pthread_t thread; 
    fprintf(stderr, "server_disk: launching a thread to listen on port %d\n", 
  ...
  while (!stopping) {
    ...
    if (total_seconds >= kMaxRunTimeSeconds) {
      fprintf(stderr, 
              "server_disk: timed out after %d minutes (safety move) ...\n", 
