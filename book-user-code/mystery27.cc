...
//
// compile with g++ -O2 -pthread mystery27.cc fancylock2.cc mutex2.cc kutrace_lib.cc dclab_log.cc -o mystery27
//
...
// The shared database
typedef struct {
  FancyLock2* readerlock[2];	// Two locks, for subscripting by account# low bit
...
// A second locks for experiment in spreadng locking around based on low bit
// of account number.
...
static int lockmod;		// Mask. 0 = single locks, 1 = multiple locks based on low bit of account#
...
int main(int argc, const char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (strcmp(argv[i], "-multilock") == 0) {lockmod = 1;}			// Use multiple reader locks
